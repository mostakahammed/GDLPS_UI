...
export class LineEdit extends Component {
  ...
  componentDidMount = async () => {
    ...
    let lineDetails = await this.getLineDetails(Id, resToken.token);

    this.setState(
      {
        id: Id,
        name: lineDetails.name,
        type: lineDetails.type.trim()
      }
    )
  }
  getLineDetails = async (Id, token) => {

    const res = await LineService.GetDetailsById(Id, token);
  ...
  handleSubmit = async (e) => {
    ...
    const res = await LineService.Edit(model, resToken.token);
...
export default withAuth(withRouter(LineEdit));
