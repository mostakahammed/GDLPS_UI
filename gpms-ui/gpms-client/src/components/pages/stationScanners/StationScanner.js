...
class StationScanner extends Component {
  constructor(props) {
    super(props);
  ...
  getData = async (pageNumber, pageSize = this.state.perPage) => {
    ...
    const res = await StationScannerService.GetListByFilter(filter, resToken.token);
  ...
  deleteRow = async (row) => {
    ...
    const res = await StationScannerService.Delete(row.id, resToken.token);
  ...
  render() {
    return (
      <div className="container-fluid">
        <Breadcrumb
          BreadcrumbParams={{
            header: "Station Scanner List",
            title: "StationScanner",
...
export default withAuth(StationScanner)
