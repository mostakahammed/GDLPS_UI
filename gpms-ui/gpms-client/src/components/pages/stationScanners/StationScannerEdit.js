...
export class StationScannerEdit extends Component {
    constructor(props) {
        super(props)
    ...
    validateFormOnLoginSubmit = () => {
        this.formValidationObject('scannerId', this.state.scannerId);
    ...
    formValidationObject = (name, value) => {
        ...
        switch (name) {
            case "scannerId":
                error.scannerId = !value.length || value === '' ? "ScannerId is Required" : "";
                break;
            case "stationId":
    ...
    componentDidMount = async () => {
        ...
        let stationscannerDetails = await this.getStationScannerDetails(Id, resToken.token);
  
        this.setState(
          {
            id: Id,
            scannerId: stationscannerDetails.scannerId,
            stationId: stationscannerDetails.stationId,
          }
        )
      }
      getStationScannerDetails = async (Id, token) => {
  
        const res = await StationScannerService.GetDetailsById(Id, token);
    ...
    handleSubmit = async (e) => {
        ...
        const res =  await StationScannerService.Edit(model, resToken.token);
...
export default StationScannerEdit
