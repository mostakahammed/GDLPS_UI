...
export class FailedMessageEdit extends Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount = async () => {
        ...
        let failedmessageDetails = await this.getFailedMessageDetails(Id, resToken.token);
        ...
        this.setState({
            ...
            id: Id,
            categoryId: failedmessageDetails.failedCategoryId,
            stationId: failedmessageDetails.stationId,
            message: failedmessageDetails.message,      
        })
    }
    ...
    validateFormOnLoginSubmit = () => {
        this.formValidationObject('categoryId', this.state.categoryId.toString());
    ...
    formValidationObject = (name, value) => {
        ...
        switch (name) {
            case "categoryId":
                error.categoryId = !value.length || value === '' ? "CategoryId is Required" : "";
                break;
            case "stationId":
                error.stationId = !value.length || value === '' ? "StationId is Required" : "";
                break;
            case "message":
                error.message = !value.length || value === '' ? "Message Order is Required" : "";
                break;
            default:
                break;
        }
      ...
      getFailedMessageDetails = async (Id, token) => {
  
        const res = await FailedMessageService.GetDetailsById(Id, token);
    ...
    handleSubmit = async (e) => {
        ...
        const { categoryId, stationId, message,id } = this.state;
        let resToken = AuthenticationService.GetToken();
        if (!resToken.isSuccess) {
            Toaster.Notify({ type: ToasterTypeConstants.Warning, message: "Warning! Session Expired. Try to login first." });
            return;
        ...
        const res = await FailedMessageService.Edit(model, resToken.token);
...
export default withAuth(withRouter(FailedMessageEdit));
