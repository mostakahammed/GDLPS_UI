...
class FailedMessage extends Component {
  ...
  getData = async (pageNumber, pageSize = this.state.perPage) => {
    ...
    const res = await FailedMessageService.GetListByFilter(filter, resToken.token);
  ...
  deleteRow = async (row) => {
    ...
    const res = await FailedMessageService.Delete(row.id, resToken.token);
...
export default withAuth(FailedMessage);
