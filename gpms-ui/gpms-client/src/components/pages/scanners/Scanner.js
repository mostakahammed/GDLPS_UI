...
class Scanner extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns: [
        ...
        {
          name: "Type",
          selector: (row) => row.type.toString(),
          sortable: true,
        },
        {
          name: "Created On",
  ...
  getData = async (pageNumber, pageSize = this.state.perPage) => {
    ...
    const res = await ScannerService.GetListByFilter(filter, resToken.token);
  ...
  deleteRow = async (row) => {
    ...
    const res = await ScannerService.Delete(row.id, resToken.token);
