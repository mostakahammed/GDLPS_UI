...
export class ScannerEdit extends Component {
  state = {
    name: "",
    type: "",
    scannerTypes: [],
  ...
  componentDidMount = async () => {
    ...
    let scannerDetails = await this.getScannerDetails(Id, resToken.token);

    this.setState(
      {
        id: Id,
        name: scannerDetails.name,
        type: scannerDetails.type,
      }
    )
  }
  getScannerDetails = async (Id, token) => {

    const res = await ScannerService.GetDetailsById(Id, token);
  ...
  handleSubmit = async (e) => {
    ...
    const res = await ScannerService.Edit(model, resToken.token);
