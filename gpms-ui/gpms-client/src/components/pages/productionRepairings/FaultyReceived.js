...
class FaultyReceived extends Component {
  constructor(props) {
    super(props);
    var today = new Date();
  ...
  getLines = async (pageNumber, pageSize = this.state.perPage, TransactionDate= this.state.date, SearchTerm= this.state.SearchTerm) => {
    ...
    let filter = { pageNumber, pageSize, IsFaultyReceived: true, TransactionDate, SearchTerm };
    this.setState({
      loading: true,
    });
  ...
  render() {
    ...
    return (
      <div className="container-fluid">
            <Helmet>
                    <meta charSet="utf-8" />
                    <title>Faulty Receive</title>
                </Helmet>
        <Breadcrumb
          BreadcrumbParams={{
            header: "Faulty Receive List",
            title: "faulty",
...
export default withAuth(FaultyReceived);
