...
class LineStation extends Component {
  constructor(props) {
    super(props);
  ...
  getData = async (pageNumber, pageSize = this.state.perPage) => {
    ...
    const res = await LineStationService.GetListByFilter(filter, resToken.token);
  ...
  deleteRow = async (row) => {
    ...
    const res = await LineStationService.Delete(row.id, resToken.token);
...
export default withAuth(LineStation)
