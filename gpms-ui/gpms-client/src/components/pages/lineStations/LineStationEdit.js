...
export class LineStationEdit extends Component {
    constructor(props) {
        super(props)
    ...
    formValidationObject = (name, value) => {
        ...
        switch (name) {
            ...
            case "stationId":
                ...
                break;
            case "sortOrder":
                error.sortOrder = !value.length || value === '' ? "SortOrder Order is Required" : "";
                break;
            default:
                break;
        }
    ...
    componentDidMount = async () => {
        ...
        let linestationDetails = await this.getLineStationDetails(Id, resToken.token);
  
        this.setState(
          {
            id: Id,
            lineId: linestationDetails.lineId,
            stationId: linestationDetails.stationId,
            sortOrder: linestationDetails.sortOrder,
          }
        )
      }
      getLineStationDetails = async (Id, token) => {
  
        const res = await LineStationService.GetDetailsById(Id, token);
    ...
    handleSubmit = async (e) => {
        ...
        const res = await LineStationService.Edit(model, resToken.token);
        ...
        if (res.response.isSuccess) {
            ...
            this.props.navigate(`/lineStation`, { replace: true });
            return;
        }
...
export default LineStationEdit
