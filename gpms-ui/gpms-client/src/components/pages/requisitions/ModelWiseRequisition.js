...
class ModelWiseRequisition extends Component {
    constructor(props) {
        super(props);
        this.state = {
            stores: [],
            ...
            columns: [
                {
                    name: "Requisition No",
                    selector: (row) => row.requisition.requisitionNo,
                    sortable: true,
                },
                // {
                //     name: "From Store",
                ...
                {
                    name: "Item Quantity",
                    selector: (row) => row.requisition.itemQuantity,
                    sortable: true,
                },
                {
                    name: "Model Name",
                    selector: (row) => row.requisition.model.name == null ? "" : row.requisition.model.name,
                    sortable: true,
                },
                {
                    name: "Type",
                    selector: (row) => row.requisition.type,
                    sortable: true,
                },
                
                {
                    name: "Requisition Date",
                    selector: (row) => row.requisition.dateInText,
                    sortable: true,
                },

                {
                    name: "Status",
                    cell: (row) => {
                        return row.requisition.requisitionStatus == "Pending" ? (
                            <p style={{ color: "Blue" }}><b>{row.requisition.requisitionStatus}</b></p>
                        ) : (
                            <p style={{ color: "Green" }}><b>{row.requisition.requisitionStatus}</b></p>
                        );
                    }
                },
                {
                ...
                // },
                {
                    ...
                    cell: (row) => {
                        return row.requisition.requisitionStatus == "Pending" ? (
                            <div>
                                <Link
                                    to={"/requisitionModel/edit/" + row.requisition.id}
    ...
    getItems = async (pageNumber, pageSize = this.state.perPage, StoreId = this.state.storeId, SearchTerm = this.state.SearchTerm) => {
        ...
        const res = await RequisitionService.GetByFilterModelWise(filter, resToken.token)
...
export default withAuth(withRouter(ModelWiseRequisition));
